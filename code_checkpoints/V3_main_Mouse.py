...
class GameState():
    def __init__(self):
        ...
        # Define units list
        self.units = []
        self.units.append(Player(self))
        self.units.append(Body(self))
        self.units.append(Food(self))
...
class Player(Unit):
    ...
    def move(self):
        ...
        while len(self.state.commandList) > 0:
            ...
            if cmd == self.direction or cmd == self.dir_opp[self.direction]:
                del self.state.commandList[0]
            else:
                self.direction = self.state.commandList[0]
                del self.state.commandList[0]
                break

        # Compute new position using direction dictionary
        for direction, value in self.direction_dict.items():
            if direction == self.direction:
                new_pos = self.position + value

        # Compute angle using angle dictionary
        for direction, value in self.angleDict.items():
            if direction == self.direction:
                self.angle = value
        ...
        bodyCount = 0
        for obj in self.state.units:
            if isinstance(obj, Body):
                bodyCount += 1
        if len(self.positionList) > bodyCount + 2:
            del self.positionList[-(bodyCount + 3)]
        ...
        if self.position == food.position:
            ...
            self.state.units.append(Body(self.state))
...
class Body(Unit):
    def __init__(self, gameState):
        ...
        # Define index of the new created body
        self.index = 1
        for obj in self.state.units:
            if isinstance(obj, Body):
                self.index += 1

        # Call Player class instance
        self.player = next(obj for obj in self.state.units if isinstance(obj, Player))

        # Try to compute position using position index, else set the position to Player's left
        try:
            self.position = self.player.positionList[-self.index-1]
        except IndexError:
            self.position = self.player.position - Vector2(self.index, 0)

        self.tile = pygame.Vector2(1, 0)
...
class Food(Unit):
    def __init__(self, gameState):
        ...
        self.position = None
        Food.move(self)
        self.tile = pygame.Vector2(2, 0)

    def move(self):
        while self.state.foodMove:
            ...
            # Units position
            for pos in self.player.positionList:
                if new_pos == pos:
                    new_pos = None
