...
class UserInterface():
    ...
    def renderWall(self, position, tile):
        # Location on screen
        ...
        if tile is not None:
            ...
            self.window.blit(self.textures, spritePoint, textureRect)
    ...
    def renderUnit(self, unit):
        ...
        if unit.angle is None:
            ...
            textureTile.blit(self.textures, (0, 0), textureRect)
            # Rotate the surface with the tile
            rotatedTile = pygame.transform.rotate(textureTile, unit.angle)
            ...
            self.window.blit(rotatedTile, spritePoint)
    ...
    def render(self):
        ...
        if self.state.gameStatus == "Play":
            ...
            for y in range(self.state.worldHeight):
                for x in range(self.state.worldWidth):
                    self.renderWall(Vector2(x,y), self.state.walls[y][x])

            # Render Score
            surface = self.fontScore.render("{}".format(self.state.score), True, (0, 0, 0))
            x = self.window.get_width() - surface.get_width() - 5
            y = 0
